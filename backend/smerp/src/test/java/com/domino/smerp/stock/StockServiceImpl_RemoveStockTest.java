...
public class StockServiceImpl_RemoveStockTest {
  ...
  void removeStock_success() {
    ...
    Stock stock1 = Stock.builder().id(1L).qty(new BigDecimal("100.000")).currentQty(new BigDecimal("600.000"))
        .location(Location.builder().id(1L).warehouse(wh1).curQty(new BigDecimal("100.000")).build())
        .build();
    wh1.addLocation(stock1.getLocation());

    Stock stock2 = Stock.builder().id(2L).qty(new BigDecimal("500.000")).currentQty(new BigDecimal("600.000"))
        .location(Location.builder().id(2L).warehouse(wh2).curQty(new BigDecimal("500.000")).build())
        .build();
    ...
    System.out.println("Result0 location curQty=" + result.get(0).getLocation().getCurQty().toPlainString());
    System.out.println("Result1 location curQty=" + result.get(1).getLocation().getCurQty().toPlainString());
    ...
    System.out.println("Result0 current qty=" + result.get(0).getCurrentQty().toPlainString());
    System.out.println("Result1 current qty=" + result.get(1).getCurrentQty().toPlainString());
