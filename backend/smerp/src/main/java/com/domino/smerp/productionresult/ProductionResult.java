...
public class ProductionResult extends BaseEntity {
    ...
    // 출발 창고
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "depart_warehouse_id", foreignKey = @ForeignKey(ConstraintMode.NO_CONSTRAINT), nullable = false)
    ...
    @JoinColumn(name = "wo_id", foreignKey = @ForeignKey(ConstraintMode.NO_CONSTRAINT))
