      ...
      <table v-else-if="workOrders.length > 0" class="table table-hover">
        ...
        <tbody>
          <tr v-for="workOrder in workOrders" :key="workOrder.id">
            ...
            <td>{{ workOrder.producedQty }}</td>
            <td>{{ workOrder.planAt ? workOrder.planAt.split('T')[0] : 'N/A' }}</td>
            <td>{{ workOrder.producedAt ? workOrder.producedAt.split('T')[0] : 'N/A' }}</td>
            <td>
              <button class="btn btn-sm btn-outline-primary" @click="goToDetail(workOrder.id)">상세보기</button>
            </td>
          </tr>
        </tbody>
      </table>
