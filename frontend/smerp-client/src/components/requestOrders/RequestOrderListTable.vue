<template>
  <div class="card shadow-sm">
    <div class="card-body">
      ...
      <table v-else-if="requestOrders.length > 0" class="table table-hover">
        <thead class="table-light">
          <tr>
            <th scope="col">발주 ID</th>
        ...
        <tbody>
          <tr v-for="requestOrder in requestOrders" :key="requestOrder.roId">
            ...
            <td>{{ requestOrder.empNo }}</td>
            <td>{{ requestOrder.companyName }}</td>
            <td>{{ requestOrder.itemName }}</td>
            <td>{{ requestOrder.totalQty }}</td>
            <td>{{ requestOrder.deliveryDate }}</td>
            <td>{{ requestOrder.status }}</td>
            <td>{{ requestOrder.createdAt }}</td>
            <td>
              <button class="btn btn-sm btn-outline-primary" @click="goToDetail(requestOrder.roId)">상세보기</button>
            </td>
          </tr>
        </tbody>
      </table>
...
async function load(page = 0) {
  ...
  error.value = null;
  try {
    const res = await fetchRequestOrders(page, 10); // Assuming page size of 10
    requestOrders.value = res.data.content;
...
function goToDetail(roId) {
  router.push({ name: 'RequestOrderDetail', params: { roId } });
}
